...
<meta name="google-site-verification" content="3OXmubfq5GtWoizewRglpwe6-nFdZSPvsfwPNHbo7lo" />
...
function loadData() {
  fetch('/api/data')
    ...
    .catch(e => { document.getElementById('alertText').textContent = 'Error loading data: ' + e.message; });
}
...
function updateDashboard(data) {
  ...
  const whoHtml = who.length > 0
    ? who.slice(0,6).map(r =>
      '<div class="signal-item ' + (r.is_zoonotic ? 'sig-high' : 'sig-info') + '">' +
        ...
        '<div><div class="signal-text">' + (r.title||'').substring(0,70) + '</div>' +
        '<div class="signal-source">WHO RSS · ' + (r.is_zoonotic ? 'ZOONOTIC' : 'OTHER') + '</div></div></div>').join('')
  ...
  const eidHtml = eid.length > 0
    ? eid.slice(0,6).map(r =>
      '<div class="signal-item ' + (r.is_zoonotic ? 'sig-critical' : 'sig-info') + '">' +
        '<div class="signal-time">' + (r.date||'').substring(0,10) + '</div>' +
        '<div><div class="signal-text">' + (r.title||'').substring(0,70) + '</div>' +
        '<div class="signal-source">CDC EID · ' + (r.is_zoonotic ? 'ZOONOTIC' : 'OTHER') + '</div></div></div>').join('')
  ...
  const srcHtml = Object.entries(sources).map(([name, info]) =>
    '<div class="source-row"><div style="display:flex;align-items:center;gap:8px">' +
      '<span class="' + (info.ok ? 'src-ok' : 'src-warn') + '">' + (info.ok ? '● OK' : '◐ WARN') + '</span>' +
      name + '</div><span class="src-num">' + (info.detail || '-') + '</span></div>').join('');
  document.getElementById('sourceStatus').innerHTML = srcHtml || '<div class="loading">No source data yet</div>';
  ...
  const modHtml = triggers.length > 0
    ? triggers.map(t =>
      '<div class="signal-item sig-high"><div class="signal-time">AUTO</div>' +
        '<div><div class="signal-text">' + t.module + '</div>' +
        '<div class="signal-source">' + t.reason + '</div></div></div>').join('')
  ...
  if (states.length > 0) {
    ...
    stateChart = new Chart(document.getElementById('stateChart').getContext('2d'), {
      type: 'bar',
      data: { labels, datasets: [{ data: vals, backgroundColor: '#00c8ff22', borderColor: '#00c8ff', borderWidth: 1, borderRadius: 2 }] },
      options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false,
        plugins: { legend: { display: false } },
        scales: { x: { ticks:{color:'#4a7090',font:{size:9}}, grid:{color:'#1a3050'} },
  ...
  const histHtml = history.length > 0
    ? history.slice(-6).reverse().map(h =>
      '<div class="signal-item sig-info"><div class="signal-time">' + (h.time || '--') + '</div>' +
        '<div><div class="signal-text">Collection run — WHO:' + (h.who||0) + ' CDC:' + (h.cdc||0) + ' Triggers:' + (h.triggers||0) + '</div>' +
        '<div class="signal-source">Auto-collector · 171 diseases monitored</div></div></div>').join('')
    : '<div class="loading">No history yet — run collector first</div>';
  document.getElementById('historyList').innerHTML = histHtml;
  document.getElementById('footerUpdate').textContent = 'Last update: ' + new Date().toLocaleTimeString();
}
...
function runCollector() {
  ...
  fetch('/api/collect', {method:'POST'})
    .then(r => r.json())
    .then(d => { btn.textContent = '▶ COLLECT NOW'; btn.disabled = false; setTimeout(loadData, 3000); })
    .catch(() => { btn.textContent = '▶ COLLECT NOW'; btn.disabled = false; });
}
    ...
    .then(data => {
      ...
      const html = reports.slice(0,8).map(r => {
        ...
        return '<div class="field-row">' +
          ...
          '<div><div style="font-weight:500;color:#e0f0ff">' + loc + '</div>' +
          '<div style="font-size:11px;color:var(--muted)">' + detail + '</div></div>' +
          '<span class="field-badge fb-' + level + '">' + score + '/100</span>' +
          '<span style="font-family:var(--mono);font-size:10px;color:var(--accent)">' + mods + ' modules</span></div>';
      }).join('');
...
function initMap() {
  if (ohMap) return;
  ohMap = L.map('map', { center: [38.5, -96.0], zoom: 4, zoomControl: true, attributionControl: false });
  L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', { maxZoom: 19 }).addTo(ohMap);
...
function updateMapMarkers(outbreaks) {
  ...
  outbreaks.forEach(ob => {
    ...
    const marker = L.marker([ob.lat, ob.lng], {icon}).addTo(ohMap).bindPopup(
      ...
      '<div style="margin-top:4px;padding:2px 6px;background:' + ob.color + '22;border:1px solid ' + ob.color + '44;display:inline-block">' + ob.level.toUpperCase() + '</div></div>',
...
def save_data(collected):
    ...
    data = {
        ...
        "sources":       sources,
        "disease_global": {
            "Global": {"cases": g.get("cases",0), "deaths": g.get("deaths",0), "active": g.get("active",0)},
...
def run_collector():
    ...
    try:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location("data_collector", "data_collector.py")
